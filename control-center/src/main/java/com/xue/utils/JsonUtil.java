...
public class JsonUtil {
    ...
    public static <T> T parseArray(File file, TypeReference<T> reference) {
        T t = null;
        try {
            t = objectMapper.readValue(file, reference);
        } catch (IOException e) {
            log.error("从文件中读取json字符串转为List列表或者Map集合失败：{}", e.getMessage());
        }
        return t;
    }
