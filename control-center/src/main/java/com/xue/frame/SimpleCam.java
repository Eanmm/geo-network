...
public class SimpleCam {
    ...
    are replaced with default values if possible. */
        ...
        if (messageId != MessageId.cam) {
            log.error("MessageID is: {} Should be: {}", messageId, MessageId.cam);
            throw new IllegalArgumentException();
        }
        ...
        if (YawRateConfidence.isMember(yawRateConfidence)) {
            ...
            log.warn("YawRateConfidence is not valid. Value={}", yawRateConfidence);
        ...
        if (this.hasLowFrequencyContainer()) {
            if (VehicleRole.isMember(vehicleRole)) {
                ...
                log.warn("VehicleRole is not valid. Value={}", vehicleRole);
    ...
    public SimpleCam(Cam camPacket) {
        ...
        if (messageId != MessageId.cam) {
            log.warn("Malformed message on BTP port 2001 from station with ID {}", stationId);
  ...
  /*
    ...
    if(!valid) log.error("{} is not valid. Value={}",
    name, value);
    return valid;
    }
  */
    ...
    public boolean isValid() {
         ...
         * approach is not very informative.*/
        if (!YawRateConfidence.isMember(yawRateConfidence)) {
            log.error("YawRateConfidence is not valid. Value={}", yawRateConfidence);
            valid = false;
        }

        if (this.hasLowFrequencyContainer() && !VehicleRole.isMember(vehicleRole)) {
            log.error("VehicleRole is not valid. Value={}", vehicleRole);
            valid = false;
        }
    ...
    public byte[] asByteArray() {
        ...
        try {
            ...
            log.error("Error converting simple CAM to byte array.", e);
