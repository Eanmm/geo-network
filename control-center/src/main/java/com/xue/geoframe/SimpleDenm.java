...
public class SimpleDenm {
    ...
    are replaced with default values if possible. */
        ...
        if (messageId != MessageId.denm) {
            log.error("Simple DENM has incorrect id. Id: {} Should be: {}", messageId, MessageId.denm);
            throw new IllegalArgumentException();
        }
        ...
        if (Termination.isMember(termination)) {
            ...
            log.warn("Termination is not valid. Value={}", termination);
        ...
        if (RelevanceDistance.isMember(relevanceDistance)) {
            ...
            log.warn("RelevanceDistance is not valid. Value={}", relevanceDistance);
        ...
        if (RelevanceTrafficDirection.isMember(relevanceTrafficDirection)) {
            ...
            log.error("RelevanceTrafficDirection is not valid. Value={}", relevanceTrafficDirection);
    ...
    public SimpleDenm(Denm denmPacket) {
        ...
        if (messageId != MessageId.denm) {
            log.warn("Malformed message on BTP port 2002 from station with ID {}", stationId);
    ...
    */
    ...
    public boolean checkInt(Class<?> classOfT, int value, String name) {
        ...
        if (intRange == null) {
            log.error("{} does not have an IntRange!", classOfT);
            return false;
        }
        if (!compareIntRange(value, intRange)) {
            log.warn("{} is outside of range. Value={}, {}", name, value, getIntRangeString(intRange));
            return false;
        } else {
            return true;
        }
    ...
    boolean isValid() {
        ...
        if (!Termination.isMember(termination)) {
            log.error("Termination is not valid. Value={}", termination);
            valid = false;
        }
        if (!checkInt(Latitude.class, latitude, "Latitude")) {
            valid = false;
        ...
        if (!RelevanceDistance.isMember(relevanceDistance)) {
            log.error("RelevanceDistance is not valid. Value={}", relevanceDistance);
            valid = false;
        }
        ...
        if (!PositioningSolutionType.isMember(positioningSolutionType)) {
            log.error("PositioningSolutionType is not valid. Value={}", positioningSolutionType);
            valid = false;
        }
